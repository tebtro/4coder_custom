...
tebtro_draw_error_annotations(Application_Links *app, Buffer_ID buffer_id, Text_Layout_ID text_layout_id, Face_ID face_id, Buffer_ID jump_buffer_id) {
    ...
    i64 last_line = -1;
    i64 code_line_number = -1;
    
    for (i32 i = 0; i < buffer_marker_count; i += 1, last_line = code_line_number) {
        code_line_number = get_line_number_from_pos(app, buffer_id, buffer_markers[i].pos);
        ...
        // @note: Remove file part of jump line.
        {
        ...
        // @note: Render annotation.
        {
            ...
            if (range_size(y) <= 0.0f) {
                continue;
            }
            
            Vec2_f32 draw_position = {
                region.x1 - get_string_advance(app, face_id, jump_line),
            ...
            if (draw_position.x < line_end_rect.x1) {
                draw_position.x = line_end_rect.x1;
            }
            
            draw_string(app, face_id, jump_line, draw_position, 0xffff0000);
        }
    }
}
